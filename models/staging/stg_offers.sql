...
validated as (
  select
    *,
    ...
    case
      when offer_id is null or trim(offer_id) = '' or lower(trim(offer_id)) = 'null' then 'MISSING_OFFER_ID'
      when bookingid_clean is null then 'MISSING_BOOKING_ID'
      when driverid_clean is null then 'MISSING_DRIVER_ID'
      when datecreated_clean is null then 'MISSING_CREATED_DATE'
      when routedistance_clean is not null and not (routedistance_clean ~ '^\d+$') then 'INVALID_ROUTE_DISTANCE'
      when state_clean is null then 'MISSING_STATE'
      when upper(trim(state)) not in ('ACCEPTED','CANCELED') then 'INVALID_STATE'
      else null
    end as error_code,

    case
      when offer_id is null or trim(offer_id) = '' or lower(trim(offer_id)) = 'null' then 'offer id is NULL/blank'
      when bookingid_clean is null then 'bookingid is NULL/blank'
      when driverid_clean is null then 'driverid is NULL/blank'
      when datecreated_clean is null then 'datecreated is NULL/blank'
      when routedistance_clean is not null and not (routedistance_clean ~ '^\d+$') then 'route distance not numeric'
      when state_clean is null then 'state is NULL/blank'
      when upper(trim(state)) not in ('ACCEPTED','CANCELED') then 'state not in ACCEPTED|CANCELED'
      else null
    end as error_detail
  from typed
)
